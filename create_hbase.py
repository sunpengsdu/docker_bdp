...
def sync_file(ip, master_flag=0):
	...
	sftp.put(localpath,remotepath)
        ...
        remotepath = "/opt/hbase-0.94.15-cdh4.7.0/conf/" + "hbase-site.xml"
        localpath  = workdir + "hbase-site.xml"
        sftp.put(localpath,remotepath)


        remotepath = "/opt/hbase-0.94.15-cdh4.7.0/conf/" + "regionservers"
        localpath  = workdir + "regionservers"
        sftp.put(localpath,remotepath)
...
if version == 'hbase-0.94.15':
	image = docker_repo + '/' + version + "-v2"
else:
	print 'BDP cannot current version of HBase'
	exit()
...
f1 = open(workdir + 'hbase-site.xml', 'w')
f1.write("<?xml version=\"1.0\"?> \n")
f1.write("<?xml-stylesheet type=\"text/xsl\"href=\"configuration.xsl\"?> \n")
f1.write("<configuration>\n")

f1.write("\t<property>\n")
f1.write("\t\t<name>hbase.rootdir</name>\n")
f1.write("\t\t<value>hdfs://"+hdfs_address+"/cluster_data/"+app_name+"/hbase" +"</value>\n")
f1.write("\t</property>\n")

f1.write("\t<property>\n")
f1.write("\t\t<name>hbase.cluster.distributed</name>\n")
f1.write("\t\t<value>true</value>\n")
f1.write("\t</property>\n")


f1.write("\t<property>\n")
f1.write("\t\t<name>hbase.zookeeper.quorum</name>\n")
f1.write("\t\t<value>"+container_pt_master[app_name + '-master']['name']+"</value>\n")
f1.write("\t</property>\n")
...
f1_region = open(workdir + 'regionservers', 'w')

for slave_name in slave_name_list:
        f1.write(container_pt_slave[slave_name]['name'] + '\n')
        f1_region.write(container_pt_slave[slave_name]['name'] + '\n')
f1.flush()
f1.close()
...
app_redis.hset('bdp_apps', app_name, 'hbase')
...
a,b,c = ssh.exec_command("/opt/hbase-0.94.15-cdh4.7.0/bin/start-hbase.sh")
