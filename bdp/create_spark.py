...
def sync_file(ip, master_flag=0):
	...
	filename_list = []
	filename_list.append('spark-env.sh')
	filename_list.append('slaves')

	for filename in filename_list:
		remotepath = "/opt/spark-1.2.0-bin-cdh4/conf/" + filename
...
if len(sys.argv) < 4:
	...
	print 'usage: [application_name] [spark_version] [worker number]'
	exit()
...
if version == 'spark-1.2':
	image = docker_repo + '/' + version + '-cdh4' + '-web'
else:
	print 'BDP cannot current version of Spark'
	exit()
...
f1 = open(workdir + 'spark-env.sh', 'w')
f1.write("SPARK_MASTER_IP="+container_pt_master[app_name + '-master']['ip']+"\n")
f1.write("SPARK_WORKER_INSTANCES=1\n")
f1.write("SPARK_WORKER_CORES=1\n")
f1.write("SPARK_WORKER_MEMORY=2g\n")
...
app_redis.hset('bdp_apps', app_name, 'spark')


ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect(container_pt_master[app_name + '-master']['ip'], username="mapred", password="111111")
a,b,c = ssh.exec_command("/opt/spark-1.2.0-bin-cdh4/sbin/start-master.sh")
print b.read()
time.sleep(1)
a,b,c = ssh.exec_command("/opt/spark-1.2.0-bin-cdh4/sbin/start-slaves.sh")
b.read()
ssh.close()
...
print 'login to the master node to start the framework, submit Spark jobs'
print 'track Spark jobs on port 8080 of master node on Web'
